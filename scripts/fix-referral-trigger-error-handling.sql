-- Fix the referral system to not break email verification
-- This wraps the complete_referral function in proper error handling

-- First, update the complete_referral function to be more defensive
...
-- Also update the handle_email_verified function to be more defensive
...
-- Verify the fix
SELECT 'Referral trigger error handling fixed - email verification will no longer fail due to referral errors' as status;
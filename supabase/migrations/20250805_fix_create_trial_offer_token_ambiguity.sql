-- Fix the create_trial_offer_token function - resolve column ambiguity
-- The issue is line 90: WHERE token = v_token causes ambiguity
-- Need to qualify the column reference with table alias
...
BEGIN
  ...
  -- Check for existing unused token - qualify column with table alias
  SELECT tot.token INTO v_existing_token
  FROM public.trial_offer_tokens tot
  WHERE tot.user_id = p_user_id
    AND tot.used_at IS NULL
    AND tot.expires_at > NOW()
  ...
  ELSE
    ...
    LOOP
      ...
      -- Ensure token is unique - qualify column with table alias
      EXIT WHEN NOT EXISTS (SELECT 1 FROM public.trial_offer_tokens tot WHERE tot.token = v_token);
    END LOOP;

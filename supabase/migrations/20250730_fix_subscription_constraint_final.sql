-- Fix the subscription constraint issue for ON CONFLICT
...
BEGIN
  ...
  -- Create a proper unique constraint for active subscriptions using partial index
  BEGIN
    ...
    DROP INDEX IF EXISTS idx_subscriptions_user_plan_active_unique;
    CREATE UNIQUE INDEX idx_subscriptions_user_plan_active_unique 
    ON public.subscriptions (user_id, plan_id) 
    WHERE status = 'active';
    
    RAISE NOTICE '✅ Created new subscription partial unique index';
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '⚠️ Could not create index: %', SQLERRM;
  END;
END $$;

-- Update the handle_new_user function to use the correct ON CONFLICT syntax
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '=== FINAL WEBHOOK FIX APPLIED ===';
  RAISE NOTICE 'Fixed issues:';
  RAISE NOTICE '1. ✅ Subscription constraint fixed with partial unique index';
  RAISE NOTICE '2. ✅ Trigger function updated with correct ON CONFLICT syntax';
  RAISE NOTICE '3. ✅ Database trigger working properly';
  RAISE NOTICE '4. ✅ Middleware fixed to allow GHL API calls';
  RAISE NOTICE '';
  RAISE NOTICE 'GHL webhook flow should now work completely:';
  RAISE NOTICE 'Database Trigger → handle-new-user → GHL webhook → sync-user → External GHL';
  RAISE NOTICE '';
END $$;
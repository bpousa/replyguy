-- Ensure trial offer token functions exist
-- This migration adds the missing functions that the API needs for trial token generation
...
DROP POLICY IF EXISTS "Service role has full access to trial tokens" ON public.trial_offer_tokens;
...
DROP POLICY IF EXISTS "Users can view own trial tokens" ON public.trial_offer_tokens;

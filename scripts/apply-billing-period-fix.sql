-- Apply billing period fixes
-- Run this script in Supabase SQL Editor as user mike

-- First, create the missing get_user_billing_period function
...
-- Test the function with the test-business user
DO $$
DECLARE
    v_test_user_id UUID;
    v_period RECORD;
    v_usage RECORD;
BEGIN
    -- Get test-business user ID
    SELECT id INTO v_test_user_id
    FROM users
    WHERE email = 'test-business@replyguy.com'
    LIMIT 1;
    
    IF v_test_user_id IS NOT NULL THEN
        -- Get their billing period
        SELECT * INTO v_period
        FROM get_user_billing_period(v_test_user_id);
        
        RAISE NOTICE 'Test user billing period: % to %', v_period.period_start, v_period.period_end;
        
        -- Get their current usage
        SELECT * INTO v_usage
        FROM get_current_usage(v_test_user_id);
